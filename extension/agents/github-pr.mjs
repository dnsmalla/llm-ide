...
//   2. Create branch `meetnotes/auto/<task-id>` from current HEAD.
//   3. Stage + commit the artifact files (which the apply step already
//      wrote under .meetnotes-auto/<task>/).
...
function execGit(repoPath, args, env = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Spreading would leak ANTHROPIC_API_KEY, MEETNOTES_JWT_SECRET,
    // MEETNOTES_VAULT_KEY, and any other secrets present in the server's
...
export async function openPullRequest({
  ...
  // changes that aren't in `.meetnotes-auto/`.  We only stage that
  ...
  // The apply step writes under `.meetnotes-auto/<task>/`; that's all
  ...
  const stagePath = path.join('.meetnotes-auto', safeTask);
  ...
  const prBody = [
    ...
    `\nReview the changes under \`.meetnotes-auto/${safeTask}/\` and decide whether to integrate them into the main tree.`,
  ].join('');
