...
// Strip URL query strings from error messages before returning — they
// may contain API keys (Backlog personal key is a query param).
function redactUrlsFromError(msg) {
  return String(msg || '')
    .replace(/\?[^\s"')]+/g, '?[REDACTED]')  // query strings
    .slice(0, 300);
}
