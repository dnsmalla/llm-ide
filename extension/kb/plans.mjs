...
/// Atomically claim a task for external dispatch. Writes `sentinel` into
/// `meta.dispatched.url` ONLY when no dispatch marker exists yet, and
/// returns true iff this caller won the claim. A concurrent caller (a
/// retryFailedDispatches() sweep racing an approved /kb/dispatch, or two
/// overlapping sweeps) sees the sentinel already present and gets false,
/// so exactly one of them proceeds to POST — closing the check-then-POST
/// race that produced duplicate GitHub/Backlog/Linear issues.
///
/// better-sqlite3 serialises writers and the SELECT+UPDATE runs inside a
/// transaction, so the read-decide-write is a single critical section.
export function claimTaskForDispatch(userId, taskId, sentinel) {
  requireUser(userId);
  const db = getDb();
  return db.transaction(() => {
    const cur = db.prepare(
    ...
    if (meta.dispatched?.url) return false; // already claimed or dispatched
    const next = { ...meta, dispatched: { url: sentinel, claimedAt: new Date().toISOString() } };
    db.prepare(
      'UPDATE plan_tasks SET meta = ? WHERE id = ? AND user_id = ?'
    ).run(safeJSONStringify(next), String(taskId), userId);
    return true;
...
/// Release a dispatch claim previously taken with `claimTaskForDispatch`.
/// Clears `meta.dispatched` ONLY when it still holds exactly `sentinel`,
/// so a stale release can never clobber a real ticket url that a
/// successful dispatch wrote over the sentinel. Returns true iff cleared.
export function releaseTaskDispatchClaim(userId, taskId, sentinel) {
  requireUser(userId);
  const db = getDb();
  return db.transaction(() => {
    const cur = db.prepare(
    ...
    if (!cur) return false;
    const meta = safeParseMeta(cur.meta) || {};
    if (meta.dispatched?.url !== sentinel) return false; // real url or nothing — leave intact
    const next = { ...meta };
