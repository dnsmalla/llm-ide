...
test('model_fallback is an allowed activity kind and round-trips', async () => {
  await freshDb();
  const { getDb } = await import('../kb/db.mjs');
  const { registerUser } = await import('../server/users.mjs');
  const { recordActivity, listActivity } = await import('../kb/activity.mjs');
  const db = getDb();
  const { id: userId } = registerUser(db, { email: 'u-mf@example.com', password: 'pw-12345678' });
  const rowId = recordActivity(db, {
    userId, kind: 'model_fallback',
    title: 'Switched gpt-4o → gpt-4o-mini (openai rate limit)',
    detail: { provider: 'openai', from: 'gpt-4o', to: 'gpt-4o-mini', reason: 'quota_429' },
  });
  assert.equal(typeof rowId, 'number');
