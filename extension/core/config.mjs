...
// Like envInt, but clamps to [min, max] instead of letting an operator typo
// (e.g. LLMIDE_MEM_TOTAL_CHARS=0) silently zero out a whole feature. Warns
// on stderr so a clamp is visible rather than a mystery regression.
function envIntClamped(name, fallback, min, max) {
  const v = process.env[name];
  if (v === undefined || v === '') return fallback;
  const n = Number(v);
  ...
  if (clamped !== n) {
    console.warn(`[config] ${name}=${n} out of range [${min}, ${max}], clamped to ${clamped}`);
  }
  return clamped;
}
