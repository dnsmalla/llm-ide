...
| `extension/kb/migrations/0001_initial.sql` through `0022_sources_user_unique.sql` | DDL deltas (22 files as of this writing). Recent: `0019_usage_limits.sql` adds `usage_ledger`, `model_limits`, and `quota_state` (model usage metering + auto-fallback — see below); `0020_issue_schedule.sql` adds the `issue_schedule` table (per-user gantt scheduling overlay for repo issues — see below); `0021_user_settings.sql` adds the `user_settings` table (NOTE: the server-side settings-sync feature it backed was reverted — the backend is localhost-only per machine; the table is **unused/vestigial**, kept because migrations are immutable); `0022_sources_user_unique.sql` recreates `sources` to widen its UNIQUE key to `(user_id, kind, ref, chunk_idx)` so two tenants indexing the same ref (e.g. a shared Box folder) no longer collide. |
