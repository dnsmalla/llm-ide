...
| `extension/kb/migrations/0001_initial.sql` through `0021_user_settings.sql` | DDL deltas (21 files as of this writing). Recent: `0018_activity.sql` adds the `activity` and `activity_seen` tables (see below); `0019_usage_limits.sql` adds `usage_ledger`, `model_limits`, and `quota_state` (model usage metering + auto-fallback — see below); `0020_issue_schedule.sql` adds the `issue_schedule` table (per-user gantt scheduling overlay for repo issues — see below); `0021_user_settings.sql` adds the `user_settings` table. NOTE: the feature this backed (a server-side settings sync) was reverted — the backend is localhost-only per machine, so it could never sync across machines. The table is now **unused/vestigial** (migrations are immutable, so it is kept, not dropped); repo/Issues/Gantt config is local per user. |
