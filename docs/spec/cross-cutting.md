...
| **Vault** | AES-256-GCM + HKDF-SHA256, 10 allow-listed keys (`vault.mjs:110–133`): `github.token`, `backlog.apiKey`, `linear.apiKey`, `slack.webhookUrl`, `email.imapPassword`, `claude.apiKey`, `openai.apiKey`, `google.apiKey`, `custom.apiKey`, `custom.baseUrl` | [`knowledge-base.md` §6](knowledge-base.md#6-vault-crypto) |
...
Migrations are numbered files applied exactly once and recorded in the `schema_migrations` table. The head migration is `0017` (`0017_slack_state.sql`). Migrations are never edited after they land; schema changes always add a new migration file.
