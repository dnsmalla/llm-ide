...
Migrations are numbered files applied exactly once and recorded in the `schema_migrations` table. The head migration is `0022` (`0022_sources_user_unique.sql`). Migrations are never edited after they land; schema changes always add a new migration file.
