...
**Session origins.** `chat-memory.origins.json` (a sidecar, never inlined into a prompt — session UUIDs are noise to the model) maps `factIndex → chat session id`, written from `agentContext.chatSessionId` (the client's stable per-conversation UUID — **not** `sessionId`, which is re-minted on every session switch). `DELETE /kb/agent/session-memory` calls `forgetSessionMemory` to drop what one chat taught, so deleting a conversation deletes its memory. An update re-attributes the row to its last writer, so deleting a chat never removes knowledge a later chat re-established, and facts captured without a session id are never removed this way. The sidecar self-prunes on read against the live fact list, which keeps it honest across the viewer's own delete/clear paths and cap eviction.
