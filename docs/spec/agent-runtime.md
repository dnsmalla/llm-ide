...
`runAgentLoop` receives a `depth` parameter (default 0). The guard at loop.mjs:150–151 throws immediately if `depth > MAX_LOOP_DEPTH` (2). Depth is incremented **once**, at loop.mjs:270, when the loop dispatches a read handler — the handler receives `ctx.depth = depth + 1` and forwards it verbatim to any nested `runAgentLoop` call. This is the single enforcement point; handler authors cannot forget the increment.
...
3. **Model call** — `runClaude(prompt, { userId, model, maxTokens })` (loop.mjs:218–223). `maxTokens` defaults to 2048 when not provided (loop.mjs:221).
4. **Parse** — `parseFence(out)` (loop.mjs:229) extracts any fence call from the raw model output (see §3).
5. **No fence** — if `parseFence` returns no fence:
   - If `parseError` is set, record it as `toolError` and continue to next iteration (loop.mjs:233–236).
   - Otherwise, return `{ reply: preToolText, pendingTool: null, iterations, cacheHits }` (loop.mjs:237).
6. **Skill lookup** — look up `fence.name` in `skills` map; unknown name → `toolError`, continue (loop.mjs:240–244).
7. **Validate args** — `validateArgs(skill.schema, fence.arguments)` (loop.mjs:246–250). Validation error → `toolError`, continue.
8. **Write skill** — if `skill.kind === 'write'`, return immediately with `{ reply: preToolText, pendingTool: { name, arguments } }` (loop.mjs:252–257). The loop does NOT execute write tools; they go to the client.
9. **Read skill cache** — key shape: `"<toolName>:<JSON-stable-args>"` (loop.mjs:261). Cache hit → return cached result, increment `cacheHits`. Cache stores only successes (loop.mjs:275). Cache capped at 100 entries; oldest entry evicted when full (loop.mjs:272–274).
10. **Read skill execution** — `runReadHandler(name, args, { userId, kb, handlers, depth: depth + 1 })` (loop.mjs:270).
11. **Handler error** → `toolError`, continue.
12. **Pending tool from nested loop** — if the handler returns `result.pendingTool`, propagate it up unchanged (loop.mjs:286–293). This is how an internal write fence surfaces to the client through the global loop.
13. **Feed result** — `toolResult = result`; next iteration embeds it as `<<<TOOL_RESULT>>>`.

When the iteration cap is exhausted, the loop returns with an iteration-limit annotation (loop.mjs:297–298).

### System prompt assembly

`buildSystemPrompt` (loop.mjs:70–93) takes `{ base, skills, agentContextBlock }` and joins them in this order:
...
|---|---|---|---|
| `name` | yes | non-empty string; must match filename (without `.md`) | loader.mjs:122–134 |
| `kind` | yes | `'read'` or `'write'` | loader.mjs:126–138 |
| `description` | no | 1–2 sentence summary; falls back to first prose line of body | loader.mjs:148–154 |
| `schema` | no | object mapping arg name → `{ type, required?, maxLength?, description? }` | loader.mjs:53–82 |
...
**Write skills require `confirmation: editable-sheet` or `confirmation: gitop-sheet`** (loader.mjs:139–142). This field is the wire contract with the client: the client receives `pendingTool` and shows a confirmation sheet before applying the change. `editable-sheet` is for file-edit and issue-create tools; `gitop-sheet` is for the `git-op` tool (which shows a git-specific confirmation UI, with a red warning banner for destructive ops).

Valid `schema` argument types: `string`, `number`, `boolean`, `string[]` (loader.mjs:12).

### Read-vs-write semantics

**Read skills** have a server-side handler function registered in `INTERNAL_HANDLERS` (ask-internal.mjs:21) or the global handler map (route.mjs:142–179). When the loop dispatches a read skill, the handler runs server-side and its result is fed back into the loop as `<<<TOOL_RESULT>>>`. The model sees the result and may make further tool calls.

**Write skills** cause the loop to exit immediately, returning `{ reply: preToolText, pendingTool: { name, arguments } }` to the caller (loop.mjs:252–257). No server-side code executes; the client is responsible for showing the editable sheet and applying the change. A write fence surfaced from a nested internal loop is propagated up through any intermediate read-handler results unchanged (loop.mjs:286–293).
...
### Per-user skill set (`buildPerUserSkillSet`, registry.mjs:202–246)

Called once per request in `handleCodeAssist` (route.mjs:56). Algorithm:
...
- `buildUserPluginHooks(userId)` (`llm_agent/skills/registry.mjs`) — hook
  handlers from plugins the user has both enabled and **hook-trusted**
  (`plugins/state.mjs` → `hooksTrusted`, default off). Only `command` handlers
  on supported events run, each with a timeout and a 4 KB output cap; exit code
  2 blocks the action, any other non-zero is a hook failure that does not stop
  the turn (`llm_agent/sdk/hooks.mjs`). The legacy CLI path and the native
  provider loop have no hook surface.
...
`composeSystemContext(agentContext, userId)` (compose.mjs:22–37) assembles the `# System context` block in this order:

1. `# System context` heading
2. `appCapabilities` — contents of `internal/context/app-capabilities.md`, cached at module load (compose.mjs:20)
...
7. `renderGraphifyMemory(agentContext, userId)` — Graphify-generated repo memory; gated on user's repo allow-list (compose.mjs:36). Each present repo's block header carries a freshness clause — `(updated ~N ago)` from the memory files' mtime — and an indexed, allow-listed repo with **no** generated memory emits an explicit `No code-graph memory generated for this repo yet.` marker instead of silently contributing nothing, so the agent can weigh or caveat stale/absent grounding (facts only — no "stale" verdict; `memory.mjs`)

   **Budget & priority order:** the block is bounded (`PER_FILE_CHARS = 6 000` per file, `TOTAL_CHARS = 40 000` total, `MAX_REPOS = 2`). `chat-memory.md` is budgeted separately in two halves — it is **read** at `chatStoreChars` (200 000, the same value the writer caps the file at, so a bullet is never sliced) but only `chatInjectChars` (16 000) of it reaches a prompt, filled by `selectChatMemoryFacts` with the facts most relevant to the current question. That gap is deliberate: the store can hold up to `maxFacts` (1 000) durable facts without per-request prompt cost growing with it. Files are added **highest-signal first** so they survive a tight budget, and the bulkier prose is what gets clipped — order: `repo.md` (impact-ranked repo overview) → `chat-memory.md` (curated durable facts from prior chats) → `graph-notes.md` → `doc-notes.md` → `bugs/` → `q&a/`. Truncation is line-boundary-safe (`clipToBoundary`, memory.mjs): when a file exceeds the remaining room it is cut back to the last newline (when that keeps >60 % of the room) and marked `…(truncated)`, so a memory fact is never split mid-sentence.
...
### Project memory capture (write half)

`persistTurnMemory` (memory-persist.mjs) runs fire-and-forget after each reply: it resolves the write target through the same `resolveAllowedRepoRoot` gate the reader uses, asks a summarize-tier model for 0–N durable facts (memory-extract.mjs), and merges them into `<repo>/system/memory/chat-memory.md` (memory-writer.mjs). Every outcome is logged as one `project_memory` line with `outcome` (`captured` / `no_facts` / `skipped` / `error`) plus counts.

**The upsert index.** Each fact may carry a stable subject id in its tag — `[category|subject-id] text` — supplied by the extractor and asked to be reused when it revises a known fact. `factIndex` returns that id when present and falls back to full-text `factKey` otherwise, and `appendChatMemory` treats a repeat index as an **update in place** (position preserved), not a duplicate to discard. Without the id, "binds to port 3456" and "binds to port 4000" have different text keys and the store held both, leaving the agent two contradictory facts with no way to tell which was current.
...
Three named tiers are resolved at server startup in `route.mjs` (route.mjs:33–35):
...
1. **Explicit `model` argument** — the caller passes a resolved model string (e.g. the value of `GLOBAL_AGENT_MODEL`). Wins immediately if truthy (route.mjs:200, ask-internal.mjs: model arg, ask-subagent.mjs:102).
2. **Plugin subagent frontmatter `model:`** — inside `askSubagent`, `subagent.model || ctx.defaultModel` is passed as the `model` arg (ask-subagent.mjs:102). The subagent's own frontmatter `model:` field wins over the deployment-wide `LLMIDE_SUBAGENT_MODEL`; when the field is absent or blank, `ctx.defaultModel` (which equals `SUBAGENT_MODEL` from route.mjs:35) is used.
3. **Tier env var** — resolved at server startup from `LLMIDE_AGENT_MODEL`, `LLMIDE_INTERNAL_MODEL`, or `LLMIDE_SUBAGENT_MODEL` (route.mjs:33–35).
...
`resolveClaudeCall({ userId, model, provider })` (runtime.mjs:111) selects the target provider before execution. `resolveProvider(model)` (providers.mjs:132) maps a model id by prefix — `claude*` → `anthropic`, `gpt*`/`o<n>`/`codex` → `openai`, `gemini*` → `google`, `deepseek*` → `deepseek`, `glm*` → `glm` — defaulting to `anthropic`. The `glm` mapping exists so a GLM id **fails loudly**: GLM has no adapter or base URL of its own (it is configured as a named Custom Provider and arrives as `custom:<uuid>`), and without the rule its ids fell through to the `anthropic` default and were answered by Claude with nothing indicating the substitution. The `custom` provider (a generic OpenAI-compatible endpoint: OpenRouter, Ollama/LM Studio, DeepSeek, …) is **not** id-prefix routable and must be selected explicitly via the `provider` argument (providers.mjs:22–31). For any non-Anthropic provider, `runClaude` calls `completeViaApi(provider, …)` — an OpenAI-compatible HTTP client (providers.mjs:229) — for the HTTP path, and `runViaCli(provider, …)` for the CLI path (runtime.mjs:115–131). A user-supplied `custom.baseUrl` is validated by an SSRF guard (`assertSafeBaseUrl`, providers.mjs:65 — https-only, rejects localhost/private IPv4+IPv6, with a DNS-resolution check) before any fetch. The per-provider API key comes from the user's vault first, then an operator env fallback (`providerApiKey`, providers.mjs:142), so spend bills the user's own account when they've configured a key.
...
### Chat-turn provider selection

Source: `extension/llm_agent/runtime/route.mjs`

`runClaude` is a single completion. A **chat turn** (`handleCodeAssist`) additionally chooses which agent loop runs, and that is a three-way decision on the turn's effective provider (`provider` from the client, else `resolveProvider(model)`):

1. **An OpenAI-compatible provider with a usable API key** → `runNativeAgentLoop`: the full tool loop (skills, file tools, approval cards) over OpenAI-style function calling. `NATIVE_PROVIDERS` is `deepseek`, `openai`, `custom`, `google`, plus any `custom:<uuid>`. Base URLs: `custom` from the vault, `deepseek` `DEFAULT_DEEPSEEK_BASE`, `google` `DEFAULT_GEMINI_OPENAI_BASE` (Gemini's OpenAI-compatible surface, which supports `tools`/`tool_choice`), `custom:<uuid>` from the provider registry.
2. **A non-Anthropic provider with no key but a logged-in CLI** (`providerHasCli` — `openai` → `codex`, `google` → `gemini`) → the turn is delegated to that CLI as a **single-shot completion** via `runClaude`. Those CLIs are their own agents and do not speak this server's tool protocol, so this server's tool loop is not in play and no `pendingTool` can be produced. An API key is what buys the full loop.

   The delegate is **rooted in the chat's workspace**: `agentContext.workspaceRoot` (tilde-expanded — Node's spawn does not expand `~`) is passed as `runClaude`'s `cwd`, through `runViaCli` to the `spawnCli`/`spawnCliStream` child. Without it the child inherited the *server's* cwd (`extension/`) and reasoned about the wrong tree. `codex` additionally receives the root explicitly as `-C <cwd>` and is pinned to `-s read-only`; the Gemini CLI has no documented read-only flag, so it keeps its own default (interactive approval, which headless can never grant). **Neither is ever given `--yolo`** — auto-approving writes is opted into per-task by the auto-task runner, never by chatting, and it matters here specifically because a write made inside the CLI could not pass through the approval cards that gate the API-key path. Pinned by `tests/providers.test.mjs`.
3. **A non-Anthropic provider with neither** (`deepseek`/`glm`/`custom`, all `cli: null`) → a provider-specific error naming what is missing. GLM's points at Settings → Custom Providers rather than asking for a key it cannot use.

Anything that resolves to `anthropic` runs the text-fence `runAgentLoop`. **Invariant:** a non-Anthropic selection must never reach that branch — it ignores `model` (it passes `GLOBAL_AGENT_MODEL`), so before this split, choosing Gemini or GLM silently returned a Claude answer. Pinned by `tests/code-assist-provider-model.test.mjs`.

### Prompt cap

A hard cap of **500 000 characters** is enforced before either path executes (runtime.mjs:82, 94–96):
...
**Trigger:** an API key is available — either from the user's vault (`getSecret(db, userId, 'claude.apiKey')`, runtime.mjs:530–536) or from `process.env.ANTHROPIC_API_KEY`.

**Prompt caching:** when `cacheTranscript` is `true` and the prompt contains the sentinel `<<<BEGIN>>>`, the prompt is split at that sentinel. The pre-sentinel portion is sent as a plain text content block; the post-sentinel portion (the fenced transcript) is sent with `cache_control: { type: 'ephemeral' }` (runtime.mjs:160–167). This cuts input-token cost by ~90% on repeated calls to long transcripts.
...
**Invocation:** the Anthropic fallback builds its env (below) and then calls `spawnCli('anthropic', prompt, { env, timeoutMs: CLAUDE_TIMEOUT_MS, signal })` (runtime.mjs:328–332). `spawnCli` (`providers.mjs:370–399`) is the single source of truth for *how* any provider CLI is invoked: it runs `execFile(bin, args, { timeout: timeoutMs, maxBuffer: 4 * 1024 * 1024, env, signal })` (providers.mjs:380–382) and closes the child's stdin (else the CLI blocks ~3 s on piped input). The argv for the Anthropic provider is `['--strict-mcp-config', '--setting-sources', '', '--tools', '', '-p', prompt]`, built by `CLI_ARG_BUILDERS.anthropic` (providers.mjs:285) via `cliInvocation` (providers.mjs:311–317). The flags serve distinct purposes:
...
**Env allowlist:** the subprocess env is built by `minimalCliEnv(extraKeys)` (`providers.mjs:328–351`) — a shared allowlist used by *every* provider CLI path so secrets like `LLMIDE_JWT_SECRET` and `LLMIDE_VAULT_KEY` are never inherited. Its base set is exactly: `PATH`, `HOME`, `TMPDIR`, `TMP`, `TEMP`, `USER`, `LOGNAME`, `SHELL`, `TERM`, `LANG`, `LC_ALL`, `NODE_ENV`, `XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `APPDATA`, `USERPROFILE`. The Anthropic fallback layers two extras on via `extraKeys` (runtime.mjs:313–316): `ANTHROPIC_BASE_URL` (from `process.env`, when set) and `ANTHROPIC_API_KEY` (only if an API key was resolved). Empty-string entries are stripped before exec (providers.mjs:349).

**CLI overload retry:** uses the same `RETRY_DELAYS_MS` schedule. A stderr/stdout match against `/\b529\b|\boverloaded\b|\b503\b|\bservice unavailable\b/i` (`isCliOverloaded`, runtime.mjs:53–56) sets `err.overloaded = true`, which triggers the retry loop (runtime.mjs:341).
...
### Usage metering & auto-fallback

`runClaude` and `completeViaApi` are instrumented against the per-user usage
ledger (`extension/kb/usage.mjs`; tables in `extension/kb/migrations/0019_usage_limits.sql`).
All writes are best-effort and must never throw into a live model call.

- **Metering.** On a successful Anthropic HTTP completion, CLI completion, and
  every `completeViaApi` (OpenAI/Google/custom) call, `recordUsage` appends a
  `usage_ledger` row (tokens when the provider reports them; CLI/subscription
  rows carry null tokens and count as one run). Anthropic `anthropic-ratelimit-*`
  and OpenAI `x-ratelimit-*` response headers are captured via `recordRateLimits`
  into an in-memory per-(user, provider) snapshot.
- **Proactive auto-fallback.** `runClaude` takes `autoFallback` (default **true**).
  When a `userId` is present it calls `resolveModel(db, userId, provider, now,
  { preferModel: model })` before dispatch: a `paused` chain throws a clear
  "all models reached their limit" error; otherwise the resolved model replaces
  the caller's only when the chain is **engaged** (a cap is set or a quota flag
  fired), so configuring no caps changes nothing. `preferModel` keeps the
  caller's model when healthy and steps **down** only when it's constrained.
- **Reactive fallback.** A non-transient `429` (quota/billing) in `completeViaApi`
  flags the model via `flagQuota`, then resolves the next chain model and retries
  once **in the same request**; the switch emits a throttled `model_fallback`
...
`dispatchPlan(userId, { planId, target, taskIds, config })` is the public entry point (dispatcher.mjs:429).

**Targets:** `preview`, `github`, `backlog`, `linear` (dispatcher.mjs:33). Any other value throws immediately. `preview` never calls an external API — it returns the exact payload that would be sent (dispatcher.mjs:277–286).
...
**Per-target concurrency:** each adapter calls `pMap(tasks, fn)` with no explicit concurrency argument, so it uses `pMap`'s default of **4** concurrent in-flight requests (p-map.mjs:19). The outcome watcher also uses `CONCURRENCY = 4` (outcome-watcher.mjs:20).

**Per-request HTTP timeout:** each external API call uses `AbortSignal.timeout(15_000)` — 15 seconds per issue-create request (dispatcher.mjs:106, 169, 237).

**Secret redaction on errors:** `redactErrorBody(text)` (dispatcher.mjs:25–31) runs all patterns from `SECRET_PATTERNS` (guardrails/rules.mjs) over the provider's error body before returning it in the result. The output is capped at 300 characters.

**Dispatch retry schedule** (dispatcher.mjs:299–305):
...
All delays are jittered ±25% via `jittered()` (backoff.mjs:12–15). Maximum retry attempts: `MAX_RETRY_ATTEMPTS = 5` (dispatcher.mjs:306). After the cap is reached, `gaveUp: true` is written to `task.meta.dispatchRetry` and no further retries are scheduled (dispatcher.mjs:326–338).

`retryFailedDispatches(userId, config)` processes at most `MAX_CONCURRENT_RETRIES = 5` tasks concurrently (dispatcher.mjs:312) and has an absolute sweep deadline of `RETRY_SWEEP_DEADLINE_MS = 120_000` ms (dispatcher.mjs:315). Tasks not reached in a sweep are deferred to the next call.
...
**Record-only-on-state-change:** `recordOutcome` (outcomes.mjs:128) wraps its INSERT in a transaction. It reads the most recent outcome row for the task and returns `null` without inserting if both `state` and `meta` are unchanged (outcomes.mjs:149–153). `pollTask` in the watcher tests `!!recorded` to set `changed` on the result (outcome-watcher.mjs:185).
...
**Per-(provider, userId) circuit breaker** (outcome-watcher.mjs:38–86):

| Parameter | Value | Source |
|---|---|---|
| `CB_FAILURE_THRESHOLD` | 3 consecutive failures | outcome-watcher.mjs:38 |
| `CB_BASE_COOLDOWN_MS` | 15 000 ms (15 s) first open | outcome-watcher.mjs:40 |
| `CB_MAX_COOLDOWN_MS` | 300 000 ms (5 min) | outcome-watcher.mjs:39 |
| Max CB entries (memory) | 10 000 | outcome-watcher.mjs:69 |

Circuit key shape: `` `${provider}::${userId}` `` (outcome-watcher.mjs:50).

Cooldown formula when `failures >= CB_FAILURE_THRESHOLD` (outcome-watcher.mjs:77–78):
...
State is in-process/in-memory and resets on server restart — by design, so a fresh start re-probes rather than staying permanently open (outcome-watcher.mjs:35–36).

When the circuit is open, `pollTask` returns a synthetic `state: 'unknown'` result with `circuitOpen: true` and does not call `pollOne` (outcome-watcher.mjs:144–156). A successful probe clears the entry entirely via `circuitBreakers.delete(k)` (outcome-watcher.mjs:63).
