...
Budgets (`Models/LoopEngine/LoopEngineConfig.swift`, persisted per loop inside `system/loop.json`): `maxIterations` (10), `consecutiveFailureStop` (2), `maxRepairsPerStage` (3), and `wallClockBudgetSeconds` (**default `nil` = no limit**; when the user sets one in Settings → Loop it is checked between iterations and skipped on the first). The time budget defaulted to 3600 and was removed as a default because spending an hour is not itself a failure — the run was progressing, and abandoning it at the hour mark discarded the work and reported `.wallClockExceeded`, which reads like a fault when nothing faulted. The remaining budgets bound the loop by **progress**, which is the property worth bounding. Per-stage `timeoutSeconds` is likewise `nil` by default (the runner's fallback `stageTimeout` is `0` = unlimited, and `ShellFaultVerifier` treats `<= 0` as unbounded), and the built-in templates ship no stage timeouts. Terminal statuses are enumerated by `Models/LoopEngine/LoopEngineStatus.swift`, whose `summary` (human) and `code` (stable, for journal grouping) are the single source of truth for every caller.
...
Persistence: the contract is `<projectRoot>/system/loop.json` via `LoopEngineConfigStore` (`Models/LoopEngine/LoopEngineConfigStore.swift`) — committed, pretty-printed + sorted keys, file-first with a one-time migration from the legacy `loopEngineConfig_<projectId>` UserDefaults entry (which `save` no longer writes). `system/loop-runs/` is in `ProjectScaffolder`'s managed `.gitignore` block plus its `managedGitignoreUpgrades` retro-fit list. Approvals (`VerifyApprovalStore`) and `LoopEngineDefaults` stay per-machine in UserDefaults.
...
Triggers, all sharing one config per project: `manual` (`Views/LoopEngine/LoopEngineView.swift`) and `autoTask` (`Services/AutoCode/AutoCodeUpdateService+PipelineTasks.swift`). `LoopRunTrigger.chat` is historical — the Code Assistant chat header used to carry a Run Loop button; it was removed, and the enum case survives only so journal records written by earlier builds still decode.
