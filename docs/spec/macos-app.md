...
`library`, `live`, `explorer`, `search`, `conflicts`, `sourceControl`, `issues`, `gantt`, `visual`, `docGen`, `autoCode`, `codeGraph`, `loopEngine`, `settings`
...
- **`mobileSyncControl`** — iPhone companion chat, explorer sessions, auto-tasks control, and Loop engine control via WebSocket on :3006 (Bonjour + PIN pairing). Excludes 15 files (`Services/MobileControlManager.swift`, `MobileWebSocketServer.swift`, `MobileBonjourAdvertiser.swift`, `MobilePin.swift`, `MobileConnectionInfo.swift`, `MobileModule.swift`, `MobileExploreBridge.swift`, `MobileExploreIndexStore.swift`, `MobileSkillCatalog.swift`, `MobileWorkspaceSearch.swift`, `PairingThrottle.swift`, `Views/Settings/MobileControlSettingsSection.swift`, `Chat/ExplorerMobileEngineResolver.swift`, `AutoTask/Services/MobileAutoTaskBridge.swift`, `LoopEngine/Services/MobileLoopBridge.swift`) and gated out the `SharedProtocol` product dependency (used only by the mobile unit). The `MobileFeatureBridge` protocol stays core to support dynamic downgrades when AutoTask/LoopEngine are compiled out (Phase 2d)

A "lite" non-engineer build (the `LLMIDE_FEATURES=agent_chat,auto_tasks,mobile_sync` preset used by `make build-mac-lite`) excludes codeGraph3D, fileExplorer, ganttIssues, docGen, and terminal, leaving only Agent Chat, Auto Tasks, Mobile Sync, Live Capture, and Library. A "minimum" build (the `LLMIDE_FEATURES=agent_chat` preset used by `make build-mac-min`) excludes all seven excludable features, leaving only Agent Chat, Live Capture, and Library.

When AutoTask/LoopEngine is compiled out, the `MobileControlManager` invokes a `replyFeatureUnavailable` path to degrade phone requests for auto-tasks and loop operations to an explicit "not installed" acknowledgement (via `MobileFeatureBridge` protocol seam). A lite or minimum install is not durable across dev rebuilds: a later full `swift build` run overwrites it with the all-features default, since nothing records the feature selection outside that one build's Info.plist.
...
**Eligibility** (`isEligible`): a `sourceRoot` (this bundle's `Info.plist` carries an `LLMIDESourceRoot` key pointing at a directory with `Package.swift`), an `installTarget` (`Bundle.main.bundleURL` ends in `.app`, not a bare `swift run` executable) must be non-nil, AND the published `toolchainAvailable` flag must be `true`. That flag starts `false` and is set from an async `xcrun --find swift` probe kicked off once from `init` (cached for the instance's lifetime) — the card cannot appear until the probe resolves, so a machine without Xcode command line tools never shows a button it can't honor. **Distributed release builds never qualify**: `Scripts/release.sh` exports `LLMIDE_OMIT_SOURCE_ROOT=1` before calling `build.sh`, which omits the `LLMIDESourceRoot` Info.plist key entirely — no local build machine path ever ships in a release bundle. Dev builds (`build_app.sh`) and staged rebuilds (`rebuild-features.sh`) call `build.sh` without that flag, so they carry it.
...
2. **`startRebuild()`** runs `mac/Scripts/rebuild-features.sh --features <active-features csv> --stage-dir <Application Support>/rebuild-staging` in a detached `Task` (staging is the script's only mode — it never swaps), streaming stdout/stderr into a 200-line internal buffer whose last 20 lines are published as `logTail`. This calls `build.sh` (release, `LLMIDE_SKIP_KILL=1`) then `sign.sh` into the stage dir; the running app is untouched.
...
- **`GraphAutoUpdater`** (`CodeGraph/GraphAutoUpdater.swift`, `@MainActor`) drives the service automatically on three triggers: project open/switch (`.activeProjectChanged`), a 15-minute timer, and a **file watcher** (below) that fires within seconds of an edit. All three funnel through `runIfEligible`, which resolves the repo to graph via **`repoToGraph(projectRoot:)`** — a `code/<repo>` **child git repo** whenever `code/` has children (preferring one already graphed for incremental stability, else the first child alphabetically). It **never graphs the project workspace root when `code/` has children**, even if the root carries a stale graph: the root isn't a git repo and holds the tool's own `system/` output, so scanning it would ingest ~20k generated files. Only a project with no `code/` children falls back to the **root itself** when it directly holds source. **First generation is automatic too** — opening a project with no graph yet generates one (the product's purpose is autonomous knowledge the user only reviews, never hand-triggers); `runIfEligible` is a no-op only for an empty project with nothing to graph. After each run it publishes the graphs into `GraphSessionStore` so the Code Graph view shows the auto-maintained result without re-scanning. Started from `AppShell` via `.task`.
...
- **`GraphSessionStore`** (`Graph/Views/GraphSessionStore.swift`, `@MainActor`) caches generated results keyed `repo#mode` (`code` / `data` / `all`) for the view's process lifetime, with a `laidOut` flag (the auto-updater stores raw graphs; the view lays them out on hydrate) and a `docFingerprint` (lets a manual InfiniteBrain re-generate reuse an unchanged doc index).

##### Graph rendering view (2D + 3D)

`UAGraphView` (`Graph/Views/UAGraphView.swift`) is the interactive graph view that hydrates from `GraphSessionStore` and renders the selected `Mode` (`code` / `data` / `all`). It supports two renderers, toggled by a `Picker` bound to `@State private var render3D` (line 135):
...
#### Model usage limits (`AutoCodeUpdateService` + Model & Limits panel)

The Auto Tasks page has a **Model & Limits** item (`AutoTask/Views/AutoCodeView.swift`) whose right pane is `AutoTask/Views/ModelLimitsPanel.swift` — a Claude-Settings-style surface: a per-model usage dashboard (progress bar + `% used` / reset), an inline cap editor (limit, runs/tokens, daily/monthly, switch-at %, enable, reorder) behind a per-row disclosure, an auto-switch status banner, and a provider-scoped API rate-limit card. It auto-refreshes every 30 s while visible (paused during edits) and reads/writes the `/kb/usage/*` routes via `Services/API/LlmIdeAPIClient+Usage.swift`.

`AutoCodeUpdateService` (`AutoTask/Services/AutoCodeUpdateService.swift`) participates in the global ledger so subscription-CLI usage is counted: before each Auto Task run it calls `GET /kb/usage/resolve` (seeded with the user's default model as `prefer`) and skips the run with a banner when the chain is `paused`; it pins the resolved model on the CLI via `--model` for `claude`/`codex`/`gemini`; and after each run it calls `POST /kb/usage/record`. The `model_fallback` activity kind (`Services/ActivityStore.swift`, mirrored from the backend `ACTIVITY_KINDS`) surfaces auto-switches in the activity bell.

#### Loop Engine (`LoopEngine/Services/`, `LoopEngine/Models/`)
...
A project holds SEVERAL independent loops (`LoopEngine/Models/LoopDefinition.swift`), persisted together as a `LoopEngineProjectStore` in `<projectRoot>/system/loop.json`. `LoopStageDetector.defaultLoops` seeds three built-ins, each marker-gated: **Regression** (fault sweep + a test verify stage), **Test** (the detected test command) and **System Check** (llm-ide's per-subsystem checks). `LoopDefinition.defaultKey` marks a built-in — it cannot be deleted (`ensureDefaultLoops` recreates it) but is fully editable; `runsOnSchedule` opts it in or out of the scheduled Auto Task, and is OPT-IN: every newly created loop starts `false`, and `LoopEngineConfigStore.normalizeScheduleOptIn` switches it off once per project for loops saved under the old default-on behavior (tracked by a per-project UserDefaults flag, so a later opt-in is never reverted). `LoopEngineConfigStore.loops(projectRoot:projectId:gitRoot:)` is the ONE entry point every surface (Loop page, scheduler, phone) resolves a project's loops through, so none of them can disagree about what exists; it also performs the one-shot migration that splits a pre-split project's single aggregate loop into the built-ins, carrying the user's edited commands, `enabled` flags and budgets across. `LoopStageDetector.ensureDefaultStages(in:gitRoot:)` is LOOP-SCOPED — a loop only ever receives the default stages its own `defaultKey` owns, and a user-created loop receives none; the pre-split aggregate overload survives only for the migration's legacy key-stamping pass. `ensureDefaultLoops` also guarantees one EDITABLE loop ("Main Loop") — seeded when the incoming list is empty and preserved when the split empties it, but never re-added afterwards, so deleting your own loop sticks — and never leaves `isPrimary` on a stage-less loop. `AutoCodeUpdateService.runLoopEngineeringSweep` runs each scheduled loop as its OWN run (own budgets, own journal record, own outcome), one at a time because of the per-git-root guard below — not chained into one pipeline; `onlyLoopId` (the phone's `loop_start` via `runSingleLoop`) and `onlyStageId` (`loop_start_stage`) narrow it to one loop or one stage, both fail-closed on an id that no longer resolves. The phone deliberately starts only the Primary loop, since `buildLoopState` reports on that one alone.

`LoopEngineRunner` (`LoopEngine/Services/LoopEngineRunner.swift`, `@MainActor`) repeats an ordered `[LoopStage]` until every stage passes or a budget stops it. Each iteration re-runs every stage from the top. A process-wide `activeRoots` set, keyed on the symlink-resolved git root, rejects a concurrent run against the same working tree whichever trigger started it (`run` returns `nil`).

Stage kinds (`LoopEngine/Models/LoopStage.swift`) — `regressionSweep` (via `RegressionSweepRunning` → `RegressionRunner`), `shellCommand` (via `FaultVerifier`, gated by `VerifyApprovalStore` in a preflight pass before any iteration), and `skill` (a generate step via `LoopSkillExecuting`; preflight also rejects a skill stage with no `skillId` chosen). Per stage: `enabled` (default `true`; a disabled stage is skipped entirely — not run, not preflighted, never gating — the escape hatch for pinned default stages, which cannot be deleted), `severity` (`blocking` | `advisory` — an advisory failure is logged and journalled but never repairs, stalls, or fails the run) and `timeoutSeconds` (override of the runner's 600 s default).

Progress + repair:

- `StageOutputParser` (`LoopEngine/Services/StageOutputParser.swift`) extracts a failing-test count from XCTest, swift-testing, `node --test`, pytest, jest, and `go test`; `nil` for unrecognised output.
- `ProgressWatch` (`LoopEngine/Models/ProgressWatch.swift`) is the single per-stage stall detector: a strictly decreasing score resets the streak, equal-or-worse increments it, and a missing score falls back to normalised output-hash equality (the pre-scoring behaviour).
- `AgentLoopStageRepairer` (`LoopEngine/Services/LoopStageRepairer.swift`) receives that delta as `RepairEvidence` and quotes it back to the agent, over the `api.codeAssist` transport with the full chat model.

`RepairScopeGuard` (`LoopEngine/Services/RepairScopeGuard.swift`) snapshots `git status --porcelain --untracked-files=all` around every repair and skill stage and compares newly-dirty paths against `GitRepairScopeGuard.defaultProtectedGlobs` (tests, build/verify config, `system/`) plus `LoopEngineConfig.extraProtectedGlobs`. `ProtectedPathPolicy` is `revert` (default) | `stop` | `warn` | `off`; under `revert`/`stop` the run terminates as `.blocked(.repairOutOfScope)` and the stage is never re-verified.

Budgets (`LoopEngine/Models/LoopEngineConfig.swift`, persisted per loop inside `system/loop.json`): `maxIterations` (10), `consecutiveFailureStop` (2), `maxRepairsPerStage` (3), and `wallClockBudgetSeconds` (**default `nil` = no limit**; when the user sets one in Settings → Loop it is checked between iterations and skipped on the first). The time budget defaulted to 3600 and was removed as a default because spending an hour is not itself a failure — the run was progressing, and abandoning it at the hour mark discarded the work and reported `.wallClockExceeded`, which reads like a fault when nothing faulted. The remaining budgets bound the loop by **progress**, which is the property worth bounding. Per-stage `timeoutSeconds` is likewise `nil` by default (the runner's fallback `stageTimeout` is `0` = unlimited, and `ShellFaultVerifier` treats `<= 0` as unbounded), and the built-in templates ship no stage timeouts. Terminal statuses are enumerated by `LoopEngine/Models/LoopEngineStatus.swift`, whose `summary` (human) and `code` (stable, for journal grouping) are the single source of truth for every caller.

Visibility: `.loopEngine` is in `ShellState.Section.userHideable`, so its top-bar button is toggleable from Settings → Menu Bar (`Views/Settings/SidebarVisibilitySection.swift`). Hiding removes the button only — the `.openSection` handler in `AppShell` sets the section directly and does not consult `config.hiddenSidebarSections`, which is what keeps Settings → Loop's Open Loop button, the menu-bar status rows and the chat command working. `redirectIfSectionHidden` still bounces to `.library` if the *current* section becomes hidden.
...
Persistence: the contract is `<projectRoot>/system/loop.json` via `LoopEngineConfigStore` (`LoopEngine/Models/LoopEngineConfigStore.swift`) — committed, pretty-printed + sorted keys, file-first with a one-time migration from the legacy `loopEngineConfig_<projectId>` UserDefaults entry (which `save` no longer writes). `system/loop-runs/` is in `ProjectScaffolder`'s managed `.gitignore` block plus its `managedGitignoreUpgrades` retro-fit list. Approvals (`VerifyApprovalStore`) and `LoopEngineDefaults` stay per-machine in UserDefaults.

Defaults: `LoopEngineDefaults` (`LoopEngine/Models/LoopEngineDefaults.swift`) stores a stage-less `LoopEngineConfig` under one app-wide UserDefaults key as the values a project inherits the first time its config is created; edited by `Views/Settings/LoopSettingsSection.swift`. All three fresh-config sites call `newConfig(stages:)`.

Templates: `LoopTemplate` (`LoopEngine/Models/LoopTemplate.swift`) is a named `LoopEngineConfig` — four built-in starters (Test & Fix, Full Verify, Skill Loop, Docs Refresh) with stable UUIDs, whose test stage carries the `detectedTestCommand` sentinel that `applied(to:)` resolves via `LoopStageDetector.detectTestCommand` (dropping the stage when nothing is detected). `applied(to:)` regenerates every stage id, since ids key `VerifyApprovalStore`. `LoopTemplateStore` (`LoopEngine/Services/LoopTemplateStore.swift`, `@MainActor @Published`) holds built-ins plus the user's saved recipes under one app-wide UserDefaults key — not per-project, unlike `LoopEngineConfig`.

Journal: `FileLoopRunJournal` (`LoopEngine/Services/LoopRunJournal.swift`) writes `<projectRoot>/system/loop-runs/<yyyy-MM>/<runId>.json` plus an append-only `index.jsonl`, alongside the `system/faults/` tree `RegressionRunner` already owns. Writes are fail-open; every exit from `run` journals. Surfaced as "Past runs" in `LoopEngine/Views/LoopEngineView.swift`.

Summary note (opt-in, `LoopEngineConfig.writeSummaryNote`): `NoteLoopRunSummaryWriter` (`LoopEngine/Services/LoopRunSummaryWriter.swift`) renders a markdown run summary — outcome, pipeline, per-stage table, files changed, protected-path violations — and saves it through `NoteService` under the `loop` note type, i.e. `llm-doc/loop/<yyyy>/<MM>/`. Returns `LoopSummaryNoteResult`; fail-open, and written after the journal.

UI: `LoopEngine/Views/LoopEngineView.swift` owns the three-pane shell and all `@State`; the detail pane's sections (Overview, Template, Selected stage, Settings, Output) live in `LoopEngine/Views/LoopEngineView+DetailPane.swift` as computed views, which is why the shared state members are internal rather than private — the same split as `CodeAssistantPanel` / `CodeAssistantPanel+Session.swift`.

Triggers, all sharing one config per project: `manual` (`LoopEngine/Views/LoopEngineView.swift`) and `autoTask` (`AutoTask/Services/AutoCodeUpdateService+PipelineTasks.swift`). `LoopRunTrigger.chat` is historical — the Code Assistant chat header used to carry a Run Loop button; it was removed, and the enum case survives only so journal records written by earlier builds still decode.
...
The `llmSession` has **no wall clock**: both `timeoutIntervalForRequest` and `timeoutIntervalForResource` are set to 30 days (`LlmIdeAPIClient.swift`), which is indistinguishable from "none" for any real session while remaining a value URLSession handles normally. They were 620 s / 15 min, sized against server deadlines that no longer exist (see [agent-runtime](agent-runtime.md)); with the server now running a turn to completion, a client-side clock would only cut the socket while the work continued, turning a slow answer into a connection error. Cancellation is the control (the chat Stop button cancels the `URLSessionTask`), and `ResourceGuardService` stops work when the machine is at risk. `authSession` keeps its short 10 s / 15 s timeouts — those are liveness probes for `/auth/*`, not work.

**`ResourceGuardService`** (`Services/ResourceGuard.swift`) is what stops long-running local work now that no deadline does. It subscribes to `DispatchSource.makeMemoryPressureSource` and stops registered jobs only when macOS reports **`.critical` pressure continuously for 30 s** (`MemoryPressureTracker`, a pure state machine so the sustained rule is unit-testable; a drop below critical clears the episode). It takes **one victim per episode, newest first** — the most recently started job is the likeliest culprit and stopping one may be enough to recover — then re-arms. **CPU load is deliberately not a trigger**: a healthy `swift build` or `npm test` pegs every core, so acting on CPU would kill exactly the work the app exists to run. Registered by `BashService`, `CodeWorkflowService`, `AutoCodeUpdateService+CLI`, `ShellFaultVerifier`, `RepoManager`'s git runner, `PluginGitInstaller`, and `MeetingNoteGenerator`; monitoring starts with the first registration and stops with the last, so an idle app pays nothing.
...
| `GET /kb/usage/limits` / `PUT /kb/usage/limits` | Read/write per-model usage caps (Model & Limits panel) |
| `GET /kb/usage/summary?provider=` | Live per-model usage + active model for the dashboard |
| `GET /kb/usage/resolve?provider=&prefer=` | Resolve the model an Auto Task run should use (skip when `paused`) |
| `POST /kb/usage/record` | Record an Auto Task CLI run into the usage ledger |
| `GET /kb/usage/ratelimits?provider=` | Latest provider API rate-limit snapshot for the gauge |
...
| File dialogs | `NSOpenPanel` (7 files) + `NSSavePanel` (2 files) | **Open** panels: `Views/Library/LibraryView.swift`, `Views/Settings/BackendSettingsSection.swift`, `Views/Shared/FileTreePanel.swift`, `Views/Shell/ProjectSwitcher.swift`, `Views/Welcome/WelcomeView.swift`, `Graph/Views/UAGraphView.swift`, `Services/NotesFolder/NotesFolderConfig.swift`. **Save** panels (`NSSavePanel`): `Views/Library/MeetingDetailView.swift`, `Views/Welcome/WelcomeView.swift` | REPLACE |
