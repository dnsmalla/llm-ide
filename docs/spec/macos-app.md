...
Budgets (`Models/LoopEngine/LoopEngineConfig.swift`, persisted per `Project.id` as UserDefaults JSON): `maxIterations` (10), `consecutiveFailureStop` (2), `maxRepairsPerStage` (3), and `wallClockBudgetSeconds` (**default `nil` = no limit**; when the user sets one in Settings → Loop it is checked between iterations and skipped on the first). The time budget defaulted to 3600 and was removed as a default because spending an hour is not itself a failure — the run was progressing, and abandoning it at the hour mark discarded the work and reported `.wallClockExceeded`, which reads like a fault when nothing faulted. The remaining budgets bound the loop by **progress**, which is the property worth bounding. Per-stage `timeoutSeconds` is likewise `nil` by default (the runner's fallback `stageTimeout` is `0` = unlimited, and `ShellFaultVerifier` treats `<= 0` as unbounded), and the built-in templates ship no stage timeouts. Terminal statuses are enumerated by `Models/LoopEngine/LoopEngineStatus.swift`, whose `summary` (human) and `code` (stable, for journal grouping) are the single source of truth for every caller.
