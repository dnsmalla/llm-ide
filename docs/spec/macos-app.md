...
| graph-kit (`GraphKit`) | vendored `1.6.1` (vendored) — `.package(path: "LocalPackages/graph-kit")`, not a git pin |
...
| File dialogs | `NSOpenPanel` (7 files) + `NSSavePanel` (2 files) | **Open** panels: `Views/Library/LibraryView.swift`, `Views/Settings/BackendSettingsSection.swift`, `Views/Shared/FileTreePanel.swift`, `Views/Shell/ProjectSwitcher.swift`, `Views/Welcome/WelcomeView.swift`, `Views/CodeGraph/UAGraphView.swift`, `Services/NotesFolder/NotesFolderConfig.swift`. **Save** panels (`NSSavePanel`): `Views/Library/MeetingDetailView.swift`, `Views/Welcome/WelcomeView.swift` | REPLACE |
