...
This mirrors the Mac client's handshake (`BackendManager.minimumServerApiVersion = 47`). The UI renders a "Server needs to be restarted" banner when `serverOnline && serverStale` is truthy (`App.tsx:507–518`). The health check repeats every `TIMING.SERVER_HEALTH_CHECK_INTERVAL_MS = 30_000` ms (`lib/config.ts:467`, `App.tsx:308`).
