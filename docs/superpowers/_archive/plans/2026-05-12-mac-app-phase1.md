...
**Architecture:** Incremental refactor behind `FeatureFlags.newShell`. Files are source of truth; thin SQLite index in `<NotesFolder>/.llmide/index.sqlite` enables fast list rendering. Server keeps unchanged endpoints; gains stateless `POST /kb/summarize` and read-only `GET /kb/export-all`. Existing per-view bodies (`TranscriptView`, `ReviewView`, `PlanView`, `SettingsView`) are reused inside the new shell with no internal changes.
...
**Working directory note:** All paths are relative to the repo root (`~/Desktop/llm-ide`). Server code lives under `extension/`. Mac code lives under `mac/Sources/LlmIdeMac/`.
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Services/FeatureFlags.swift`
- Create: `mac/Sources/LlmIdeMac/Services/ShellState.swift`
...
And in the `LlmIdeMac` target's `dependencies` array add:
...
/// `LLMIDE_NEW_SHELL` (set to "1" or "0") lets QA test either side
/// without a rebuild.
enum FeatureFlags {
    static var newShell: Bool {
        if let override = UserDefaults.standard.string(forKey: "LLMIDE_NEW_SHELL") {
            return override == "1"
        }
        #if DEBUG
        return true
...
```bash
git add mac/Package.swift mac/Package.resolved mac/Sources/LlmIdeMac/Services/FeatureFlags.swift mac/Sources/LlmIdeMac/Services/ShellState.swift
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Models/MeetingFrontmatter.swift`
- Create: `mac/Tests/LlmIdeMacTests/FrontmatterCoderTests.swift`
- Create: `mac/Sources/LlmIdeMac/Services/NotesFolder/FrontmatterCoder.swift`
...
```bash
git add mac/Sources/LlmIdeMac/Models/MeetingFrontmatter.swift \
        mac/Sources/LlmIdeMac/Services/NotesFolder/FrontmatterCoder.swift \
        mac/Tests/LlmIdeMacTests/FrontmatterCoderTests.swift
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Services/NotesFolder/NotesFolderConfig.swift`

- [ ] **Step 1: Write the failing test**

Append to `mac/Tests/LlmIdeMacTests/FrontmatterCoderTests.swift` (we'll move it to its own file later — keeping it co-located while small):

Actually create new file `mac/Tests/LlmIdeMacTests/NotesFolderConfigTests.swift`:
...
final class NotesFolderConfig {
    ...
    private let bookmarkKey  = "LLMIDE_NOTES_FOLDER_BOOKMARK"
    private let pathKey      = "LLMIDE_NOTES_FOLDER_PATH"
...
```bash
git add mac/Sources/LlmIdeMac/Services/NotesFolder/NotesFolderConfig.swift \
        mac/Tests/LlmIdeMacTests/NotesFolderConfigTests.swift
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Services/NotesFolder/MeetingFileStore.swift`
- Create: `mac/Tests/LlmIdeMacTests/MeetingFileStoreTests.swift`
...
Create `mac/Sources/LlmIdeMac/Models/MeetingSummary.swift`:
...
```bash
git add mac/Sources/LlmIdeMac/Models/MeetingSummary.swift \
        mac/Sources/LlmIdeMac/Services/NotesFolder/MeetingFileStore.swift \
        mac/Tests/LlmIdeMacTests/MeetingFileStoreTests.swift
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Services/NotesFolder/MeetingIndex.swift`
- Create: `mac/Tests/LlmIdeMacTests/MeetingIndexTests.swift`

- [ ] **Step 1: Add SQLite dependency**

The existing Mac app already links against SQLite via system framework (check `Package.swift` — if not present, the `LlmIdeMac` target needs `linkerSettings: [.linkedLibrary("sqlite3")]`). Verify with:
...
```bash
git add mac/Sources/LlmIdeMac/Services/NotesFolder/MeetingIndex.swift \
        mac/Tests/LlmIdeMacTests/MeetingIndexTests.swift \
        mac/Package.swift
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Services/NotesFolder/FolderIndexer.swift`
- Create: `mac/Tests/LlmIdeMacTests/FolderIndexerTests.swift`
...
```bash
git add mac/Sources/LlmIdeMac/Services/NotesFolder/FolderIndexer.swift \
        mac/Tests/LlmIdeMacTests/FolderIndexerTests.swift
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Services/NotesFolder/PartialRecovery.swift`
- Create: `mac/Tests/LlmIdeMacTests/PartialRecoveryTests.swift`
...
```bash
git add mac/Sources/LlmIdeMac/Services/NotesFolder/PartialRecovery.swift \
        mac/Tests/LlmIdeMacTests/PartialRecoveryTests.swift
...
**Files:**
- Modify: `mac/Sources/LlmIdeMac/Services/LiveSessionMirror.swift`

- [ ] **Step 1: Read the current file**

Run: `cat mac/Sources/LlmIdeMac/Services/LiveSessionMirror.swift | head -80`
...
Run: `cd mac && ./build_app.sh && open mac/LlmIdeMac.app`
...
```bash
git add mac/Sources/LlmIdeMac/Services/LiveSessionMirror.swift
...
const MODEL = process.env.LLMIDE_SUMMARIZE_MODEL || 'claude-opus-4-7';
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Views/AppShell.swift`
- Create: `mac/Sources/LlmIdeMac/Views/Shell/SidebarView.swift`
- Modify: `mac/Sources/LlmIdeMac/Views/ContentView.swift`
- Create: `mac/Tests/LlmIdeMacTests/AppShellTests.swift`
...
In `mac/Sources/LlmIdeMac/Views/ContentView.swift`, replace the `authenticatedShell` body with:
...
Run: `cd mac && swift build`
Then: `./build_app.sh && open LlmIdeMac.app`
...
git add mac/Sources/LlmIdeMac/Views/AppShell.swift \
        ...
        mac/Sources/LlmIdeMac/Views/Library/LibraryView.swift \
        mac/Sources/LlmIdeMac/Views/Library/MeetingDetailView.swift \
        mac/Sources/LlmIdeMac/Views/ContentView.swift \
        mac/Tests/LlmIdeMacTests/AppShellTests.swift
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/ViewModels/LibraryViewModel.swift`
- Modify: `mac/Sources/LlmIdeMac/Views/Library/LibraryView.swift`
- Create: `mac/Sources/LlmIdeMac/Views/Library/LibraryRow.swift`
- Create: `mac/Tests/LlmIdeMacTests/LibraryViewModelTests.swift`
    ...
    private func load() async {
        do {
            ...
            let idx = try MeetingIndex(
                url: config.currentFolder.appendingPathComponent(".llmide/index.sqlite"))
            let vm = LibraryViewModel(index: idx)
            try vm.refresh()
            self.vm = vm
        } catch {
...
```bash
git add mac/Sources/LlmIdeMac/ViewModels/LibraryViewModel.swift \
        mac/Sources/LlmIdeMac/Views/Library/LibraryRow.swift \
        mac/Sources/LlmIdeMac/Views/Library/LibraryView.swift \
        mac/Tests/LlmIdeMacTests/LibraryViewModelTests.swift
...
**Files:**
- Modify: `mac/Sources/LlmIdeMac/Services/LlmIdeMacApp.swift` (or `AppDelegate.swift`)
- Modify: `mac/Sources/LlmIdeMac/Views/AppShell.swift`
- Modify: `mac/Sources/LlmIdeMac/ViewModels/LibraryViewModel.swift`

- [ ] **Step 1: Add an `AppEnvironment` holder**

Create `mac/Sources/LlmIdeMac/Services/AppEnvironment.swift`:
...
final class AppEnvironment {
    ...
    init() throws {
        ...
        try FileManager.default.createDirectory(at: folder, withIntermediateDirectories: true)
        let idx = try MeetingIndex(
            url: folder.appendingPathComponent(".llmide/index.sqlite"))
...
git add mac/Sources/LlmIdeMac/Services/AppEnvironment.swift \
        mac/Sources/LlmIdeMac/Services/NotesFolder/FolderIndexer.swift \
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/ViewModels/MeetingDetailViewModel.swift`
- Modify: `mac/Sources/LlmIdeMac/Views/Library/MeetingDetailView.swift`
- Create: `mac/Sources/LlmIdeMac/Views/Library/SummarySections.swift`
- Create: `mac/Tests/LlmIdeMacTests/MeetingDetailViewModelTests.swift`
...
(`LlmIdeAPIClient.summarize` is added in Task 16.)
...
```bash
git add mac/Sources/LlmIdeMac/ViewModels/MeetingDetailViewModel.swift \
        mac/Sources/LlmIdeMac/Views/Library/SummarySections.swift \
        mac/Sources/LlmIdeMac/Views/Library/MeetingDetailView.swift \
        mac/Tests/LlmIdeMacTests/MeetingDetailViewModelTests.swift
...
## Task 16: `LlmIdeAPIClient.summarize` — call `/kb/summarize`

**Files:**
- Modify: `mac/Sources/LlmIdeMac/Services/LlmIdeAPIClient.swift`
...
```bash
git add mac/Sources/LlmIdeMac/Services/LlmIdeAPIClient.swift \
        mac/Sources/LlmIdeMac/Services/LiveSessionMirror.swift
...
```bash
git add mac/Sources/LlmIdeMac/Views/Shell/SidebarView.swift \
        mac/Sources/LlmIdeMac/Views/AppShell.swift
...
- Modify: `mac/Sources/LlmIdeMac/Views/AppShell.swift`
- Modify: `mac/Sources/LlmIdeMac/Views/Shell/SidebarView.swift`
- Modify: `mac/Sources/LlmIdeMac/Views/Library/*`
...
In `LlmIdeMacApp.swift`'s `WindowGroup` (or `Window`) modifier chain add:
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Services/LegacyExporter.swift`
- Create: `mac/Tests/LlmIdeMacTests/LegacyExporterTests.swift`
...
```bash
git add mac/Sources/LlmIdeMac/Services/LegacyExporter.swift \
        mac/Tests/LlmIdeMacTests/LegacyExporterTests.swift
...
**Files:**
- Modify: `mac/Sources/LlmIdeMac/Services/LlmIdeAPIClient.swift`
- Create: `mac/Sources/LlmIdeMac/Views/Recovery/LegacyExportPromptView.swift`
- Modify: `mac/Sources/LlmIdeMac/Views/AppShell.swift`

- [ ] **Step 1: Add NDJSON streamer**

In `LlmIdeAPIClient.swift`:
...
@AppStorage("LLMIDE_LEGACY_PROMPT_SUPPRESSED") private var suppressed = false
...
```bash
git add mac/Sources/LlmIdeMac/Services/LlmIdeAPIClient.swift \
        mac/Sources/LlmIdeMac/Views/Recovery/LegacyExportPromptView.swift \
        mac/Sources/LlmIdeMac/Views/AppShell.swift
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Views/Settings/NotesFolderSection.swift`
...
```bash
git add mac/Sources/LlmIdeMac/Views/Settings/NotesFolderSection.swift \
        mac/Sources/LlmIdeMac/Views/SettingsView.swift
...
**Files:**
- Create: `mac/Sources/LlmIdeMac/Views/Recovery/RecoveryPromptView.swift`
- Modify: `mac/Sources/LlmIdeMac/Views/AppShell.swift`
...
```bash
git add mac/Sources/LlmIdeMac/Views/Recovery/RecoveryPromptView.swift \
        mac/Sources/LlmIdeMac/Views/AppShell.swift
...
**Files:**
- Modify: `mac/Sources/LlmIdeMac/Services/FeatureFlags.swift`
...
```bash
defaults write co.gridpredict.llmidemac LLMIDE_NEW_SHELL 0
...
Then:
```bash
defaults write co.gridpredict.llmidemac LLMIDE_NEW_SHELL 1
open mac/LlmIdeMac.app
```
Expected: new shell.
...
Then unset:
```bash
defaults delete co.gridpredict.llmidemac LLMIDE_NEW_SHELL
open mac/LlmIdeMac.app
```
...
```bash
git add mac/Sources/LlmIdeMac/Services/FeatureFlags.swift
git commit -m "feat(mac): make new shell the release default

Old shell remains accessible via LLMIDE_NEW_SHELL=0 UserDefaults
...
**Files:**
- Modify: `mac/Sources/LlmIdeMac/Views/ContentView.swift`
- Delete: `mac/Sources/LlmIdeMac/Views/HistoryView.swift`
- Modify: `mac/Sources/LlmIdeMac/Services/LiveSessionMirror.swift`
- Modify: `mac/Sources/LlmIdeMac/Services/FeatureFlags.swift`
...
```bash
git rm mac/Sources/LlmIdeMac/Views/HistoryView.swift
```
