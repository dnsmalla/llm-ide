...
**Working directory.** All paths are relative to the repo root `/Users/dinesh.malla/Desktop/llm-ide`.
...
```yaml
site_name: LLM IDE — Engineering Docs
site_description: Internal engineering documentation for the LLM IDE system.
...
Make LLM IDE capture captions from a platform the scraper doesn't yet know about.
...
Open `extension/server/config.mjs`. Note every call site of `envStr(...)`, `envInt(...)`, `envBool(...)`. Each takes `(name, fallback)`. Names look like `LLMIDE_*` or `PORT`. The surrounding comment block (above each section) is the human description.
...
def test_extract_finds_known_envs(tmp_path: Path) -> None:
    ...
    src.write_text(
        ...
        // LLMIDE_JWT_SECRET - signs access + refresh tokens.
        const JWT = envStr('LLMIDE_JWT_SECRET', 'devsecret');
        const FLAG = envBool('LLMIDE_LOG_JSON', false);
        """
    )
    rows = extract(src)
    names = {r["name"] for r in rows}
    assert names == {"PORT", "LLMIDE_JWT_SECRET", "LLMIDE_LOG_JSON"}
    by_name = {r["name"]: r for r in rows}
    assert by_name["PORT"]["type"] == "int"
    assert by_name["PORT"]["default"] == "3456"
    assert by_name["LLMIDE_LOG_JSON"]["type"] == "bool"
```
...
Create `docs/_scripts/extract_env_vars.py`:
```python
"""Extract LLMIDE_* env vars from extension/server/config.mjs.
