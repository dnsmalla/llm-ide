...
1. **Agent-loop hard deadline (MED).** ✅ Addressed (2026-06-23, follow-up session). `runAgentLoop` now derives a per-call `AbortSignal.timeout(remaining)` from the deadline and passes it to `runClaude`; `runClaude` combines it with its per-attempt 60s timeout (`AbortSignal.any`), and a caller-signal abort propagates immediately instead of retrying or falling through to the CLI (CLI `execFile` also gets the signal). An in-flight overrun returns the same graceful deadline reply as the between-iteration check. Tested in `agent-loop-deadline.test.mjs` (in-flight abort bails at the deadline, not the full call). `npm test` 466/466, eslint clean.
...
3. **Secret-redaction parity on the Mac side.** ✅ Addressed (2026-06-23, follow-up session). Added `mac/.../Utilities/SecretRedactor.swift` mirroring the extension's `core/redact-secrets.mjs` pattern set; `RepoManager.redact` now runs surfaced error text through it after the known-token replacement, so an unknown `ghp_`/`sk-ant-`/Bearer credential git echoes is also scrubbed. Unit tests in `SecretRedactorTests.swift`. `swift build` green; tests must run on CI/full Xcode (no `xctest` runner on the dev box).
4. **GraphAutoUpdater lifecycle (MED, from the earlier architecture review).** ✅ Addressed (2026-06-23, follow-up session). `AppShell` now calls the existing `stop()` via `.onDisappear`; since `ContentView` only mounts `AppShell` while signed in (it swaps in `LoginView` on logout), teardown is the logout signal — the 15-min timer no longer keeps scanning after logout. `swift build` green; GUI lifecycle behavior needs runtime confirmation in the real app. (The disjoint-instance issue from that review was already fixed and shipped earlier this session.)
5. **Read-tool cache key (LOW).** ✅ Addressed (2026-06-23, follow-up session). `loop.mjs` now keys the read-result cache on a new `stableStringify` (recursively sorts object keys; preserves array order) instead of `JSON.stringify(args)`. Stability was previously masked by `validateArgs` happening to reconstruct args in schema order — a hidden coupling; the cache key is now self-contained, so the spec's "stable" claim is provably true. Unit tests in `agent-loop-cache-key.test.mjs`.
