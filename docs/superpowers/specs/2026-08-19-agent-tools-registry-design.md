...
`toolCtx` is the same shape route.mjs already builds per-request: `{ userId, agentContext, runClaude, kb, userSkills, userSubagents, sessionId, requestApproval }`. `project_memory` moves in as a first-class `kind: 'read'` entry — legacy gains it as a side effect (parity fix, see §9), which means it ALSO needs a new `llm_agent/global/project_memory.md` file (schema/description live there, per the rule above; see §9). **`kb_search` and legacy's `search-kb` are the same function under two names** (both call `kb.search(userId, {q, kind, limit})`; v2's version is a near-duplicate written during the P1 spike, exactly the two-place-drift class this spec exists to close) — they unify into ONE registry entry named `search-kb` (the name with an existing skill doc and established prompt-facing contract, whose `.md` gets a `limit` param added to match `kb_search`'s richer schema), adopting `kb_search`'s `{hits, total}` shape; v2's SDK tool is exposed as `mcp__llmide__search-kb`, not a separately-named `kb_search`.
...
- **Auto-safe allowlist** (run-bash only) — a fixed prefix/pattern list: `git status`, `git diff`, `git log`, `ls`, `cat`, `grep`/`rg` — matched against the command string, conservative (unmatched commands fall through to prompt, never silently allowed). *Amended 2026-09-05 (security audit):* a matching prefix is necessary but not sufficient — the command also drops to prompt if it contains shell control or unquoted expansion syntax, names a denied path (`isDeniedPath`, case-insensitive), an over-broad root, or a private area of `$HOME`, judged after resolving relative tokens against the command's real cwd. `find` was removed earlier (`-delete` bypass). Test runners (`npm test`, `swift test`, `node --test`) execute the repo's own code, so they are auto **only** when the cwd is inside a DB-trusted indexed repo (`buildTrustedRoots`) — never the merely-opened workspace. Registering a repo is the trust decision: like `buildReadableRoots`, indexed roots are honored as-is, so a user who registers `$HOME` or `/` has opted into test runners being auto beneath it (the cwd itself still cannot be `$HOME` or `/`).
...
**Always-allow persistence** — new migration `0030_tool_approvals.sql`: `tool_approvals(user_id, tool_name, created_at, PRIMARY KEY(user_id, tool_name))`. The gate always runs FIRST, on every invocation, regardless of any row here — `blocked` and `auto` are resolved unconditionally before always-allow is ever consulted. A row only ever short-circuits the `prompt` tier's interactive round-trip (skip straight to running once approval would otherwise be asked). This ordering is load-bearing: checking always-allow before the gate would let a tool a user once approved for one safe invocation bypass the blocked check forever after, on any later — including genuinely destructive — invocation of the same tool name. The Mac approval card answers `allow` (once) / `deny` / `always-allow` (writes the row, consulted only at the `prompt` tier).
...
- `project_memory` becomes available to the **legacy** engine too (today v2-only) — a registry entry plus a new `llm_agent/global/project_memory.md` skill doc (see §4's note on legacy prompt docs) so the model actually learns it exists.
