...
- Auth ladder (first-party): vault `claude.apiKey` → `ANTHROPIC_API_KEY` env → operator ambient login (`allowAmbientAuth`). Gateway turn (`provider = custom:<uuid>` with `anthropicBaseURL`): the provider's own vault key + `ANTHROPIC_BASE_URL`, no ambient rung; any other provider is refused before spawn (`resolveAgentEngineAuth`). The engine home follows the user's first-party auth in both cases (per-user `CLAUDE_CONFIG_DIR` iff the user has a Claude key), so Claude and gateway turns of one user share transcripts
...
- Provider picker narrows to agent-capable providers (Claude + custom providers with `anthropicBaseURL`) when v2 is on. Sessions UI unchanged. Switching a v2 chat between Claude and a custom provider resumes the SAME SDK session: the engine home (`CLAUDE_CONFIG_DIR`) is a property of the user's first-party Claude auth (vault/env key → per-user home; ambient login → operator home), not of the turn, so all of one user's turns share one transcript directory and the SDK carries the conversation across the model change. (Deciding the home per turn — "keyed → per-user home" — made an ambient user's gateway turn change homes, miss the resume, and let the route's fresh-turn cleanup delete the Claude transcript.)
