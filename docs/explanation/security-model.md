...
- hooks declared in `hooks/hooks.json` run only for a plugin the user has explicitly **hook-trusted** (`plugin.hooks.trust`, audited; default off, revocable, pruned on uninstall). Only `command` handlers on supported events run, bounded by a per-handler timeout and a 4 KB output cap, and a hook command that resolves outside its own plugin directory is refused at parse time. Hook execution is v2-engine-only.
