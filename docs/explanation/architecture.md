...
SQLite, WAL mode, FTS5, foreign keys on. There is no single schema-version number — each migration under `extension/kb/migrations/` (currently `0001`–`0017`) is recorded in the `schema_migrations` table and applied on server start. The head migration is the effective schema version.
