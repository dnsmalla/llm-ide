...
4. **Mac linker — only if the wire had to change** (new event type, new tool name, new approval arg): `ClaudeLink/AgentV2Event.swift`, `ClaudeToolPresentation.swift` (verb/title rows for new tools), `AgentV2Transport.swift`; a renamed CLI flag or model line lands in `ClaudeLink/ClaudeCLI.swift`. Bump `SERVER_API_VERSION` for **any** change to the wire format or the `ENDPOINTS` list (that list drives the client's stale-server banner) — see `invariants.md`. Raise `BackendManager.minimumServerApiVersion` only when an OLDER server would make a chat-path capability silently degrade or run with the wrong credentials; additive, off-chat changes leave the floor alone (v44, v45 and v46 all did, each with its reason recorded next to the constant). The two are therefore NOT in lockstep. v47 also declines to raise it, but for a different reason than v44-v46: the degradation is real (an older server resolves the unified quick chat's `ask` mode to `execute`, handing full act tools to a window whose approvals may have nothing to render), but it is confined to the three surfaces that actually send `ask` — so gate at the sending surface instead of the app-wide floor: `Chat/QuickChatContext.swift`'s `serverSupportsAsk` is what `MenuBarChatView`, `LlmChatSheet` and `MobileControlManager`'s `llmide_chat` handler each check before sending.
