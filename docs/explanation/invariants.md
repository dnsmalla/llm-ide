...
- **`LoopEngineConfig` encodes `wallClockBudgetSeconds` as an explicit null, via a hand-written `encode(to:)`.** `nil` means "no time limit" — a value the user can choose — while an *absent* key means "written before the field existed" and must decode as 3600. The synthesized encoder omits nil optionals, which collapsed the two and silently restored a chosen "no limit" as 60 minutes on the next load. Any future optional field with a non-nil default needs the same treatment.
