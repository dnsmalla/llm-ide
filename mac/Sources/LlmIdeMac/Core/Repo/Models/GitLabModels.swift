...
struct GitLabUser: Identifiable, Codable, Hashable {
    ...
    init(id: Int, username: String, name: String, avatarUrl: String?) {
        self.id = id; self.username = username; self.name = name; self.avatarUrl = avatarUrl
    }

    // Tolerant decode: a deleted/service account can have a missing or null
    // `name` (and rarely `username`). Default rather than throw, so one such
    // author doesn't fail the whole issue-list decode and blank the UI.
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        self.id = (try? c.decode(Int.self, forKey: .id)) ?? 0
        let user = (try? c.decodeIfPresent(String.self, forKey: .username)) ?? nil
        let display = (try? c.decodeIfPresent(String.self, forKey: .name)) ?? nil
        self.username = user ?? "ghost"
        self.name = display ?? user ?? "(deleted user)"
        self.avatarUrl = (try? c.decodeIfPresent(String.self, forKey: .avatarUrl)) ?? nil
    }
}
...
struct GitLabIssuePayload: Encodable {
    // Optional so a partial update (e.g. just closing an issue or changing
    // labels) OMITS the title rather than serializing "" and WIPING it
    // server-side. Synthesized Encodable uses encodeIfPresent for optionals.
    var title: String?
    var description: String?
