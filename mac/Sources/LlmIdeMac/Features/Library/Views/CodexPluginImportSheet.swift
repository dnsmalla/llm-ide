...
/// Sheet for browsing and importing OpenAI Codex CLI plugins. Mirrors
/// ClaudePluginImportSheet — same two-tab (Installed/Marketplace) shape,
/// same Import/Update semantics — pointed at the Codex Plugin Bridge
/// instead (server/auth-routes.mjs, plugins/codex-adapter.mjs).
struct CodexPluginImportSheet: View {
    ...
    @State private var installedPlugins: [CodexPlugin] = []
    @State private var marketplacePlugins: [CodexMarketplacePlugin] = []
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if loading {
                Spacer()
                ProgressView("Scanning Codex plugins\u{2026}")
                    .controlSize(.small)
                Spacer()
    ...
    private var header: some View {
        HStack {
            VStack(alignment: .leading, spacing: 2) {
                Text("Import from Codex")
                    .font(.headline)
                Text("Browse and import plugins from your Codex CLI installation")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Spacer()
            ...
            if filteredInstalled.isEmpty {
                if !searchText.isEmpty {
                    ...
                    emptyState(
                        ...
                        subtitle: "Install plugins in Codex CLI first (/plugins), then import them here."
                    )
                }
            } else {
            ...
            if filteredMarketplace.isEmpty {
                if !searchText.isEmpty {
                    ...
                    emptyState(
                        ...
                        subtitle: "Open Codex CLI to sync its plugin marketplace, then come back here."
                    )
                }
            } else {
    ...
    private func installedRow(_ plugin: CodexPlugin) -> some View {
        HStack(spacing: 10) {
    ...
    private func marketplaceRow(_ plugin: CodexMarketplacePlugin) -> some View {
        HStack(spacing: 10) {
            Image(systemName: "puzzlepiece.extension")
                .foregroundStyle(plugin.installedInCodex ? .teal : .secondary)
                .frame(width: 20)
            VStack(alignment: .leading, spacing: 1) {
                HStack(spacing: 6) {
                    Text(plugin.name).font(.callout.weight(.medium))
    ...
    private func load() async {
        ...
        error = nil
        do {
            async let i = try api.listCodexInstalled()
            async let m = try api.listCodexMarketplace()
            let (iResp, mResp) = try await (i, m)
            installedPlugins = iResp.plugins
            marketplacePlugins = mResp.plugins
        } catch {
            self.error = "Could not scan Codex plugins: \(error.localizedDescription)"
        }
        loading = false
    }
    ...
    private func doImport(name: String, source: String, isUpdate: Bool) async {
        ...
        importMessage = nil
        do {
            let resp = try await api.importCodexPlugin(name: name, source: source)
