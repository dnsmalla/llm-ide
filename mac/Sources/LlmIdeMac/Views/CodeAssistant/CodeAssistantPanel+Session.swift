...
extension CodeAssistantPanel {
    ...
    func deleteSession(_ id: UUID) {
        ...
        // don't keep coming back in every later prompt. Fire-and-forget: the
        // chat file is already gone either way, so a failure here is
        // recoverable by a later delete rather than something to block the
        // UI on. Not repo-scoped, so no project/workspace context to capture.
        let api = self.api
        Task.detached {
