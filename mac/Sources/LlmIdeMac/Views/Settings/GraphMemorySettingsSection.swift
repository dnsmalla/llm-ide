...
/// Read-only status (generation happens in the Code Graph view; the auto-update
/// cadence lives in Auto Tasks) — this just answers "is there any data?".
...
struct GraphMemoryState {
    ...
    static func read(projectRoot: URL?) -> GraphMemoryState {
        ...
        // Resolve the repo that actually holds the graph (project root, else the
        // first code/<child> with one) — mirrors GraphAutoUpdater.existingGraphRepo,
        // inlined via ProjectLayout (a plain struct) to stay nonisolated.
        func graphed(_ root: URL) -> Bool {
            fm.fileExists(atPath: ProjectLayout(root: root).graphDir.appendingPathComponent("index.md").path)
        }
        let repo: URL = {
            if graphed(projectRoot) { return projectRoot }
            let codeDir = ProjectLayout(root: projectRoot).codeDir
            let children = (try? fm.contentsOfDirectory(at: codeDir,
                includingPropertiesForKeys: [.isDirectoryKey])) ?? []
            return children.first(where: graphed) ?? projectRoot
        }()
