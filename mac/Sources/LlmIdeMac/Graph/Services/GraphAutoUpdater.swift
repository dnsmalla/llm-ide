...
final class GraphAutoUpdater: ObservableObject {
    ...
    /// Floored at 5 min; reschedules the running timer so it takes effect
    /// immediately rather than only on next launch.
    func setIntervalMinutes(_ minutes: Int) {
        let secs = TimeInterval(max(5, minutes) * 60)
        guard secs != intervalSeconds else { return }
    ...
    /// Logic moved to core's RepoGraphLocator so the auto-task pipeline can
    /// consume the mapping even when the Graph feature is excluded.
    static func repoToGraph(projectRoot: URL) -> URL? {
        RepoGraphLocator.repoToGraph(projectRoot: projectRoot)
    }
}
