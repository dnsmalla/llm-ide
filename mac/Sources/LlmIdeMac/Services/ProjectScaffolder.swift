...
enum ProjectScaffolder {
    ...
    /// Entries added to the managed block after it first shipped, each paired with
    /// a sentinel whose absence means the upgrade has not been applied yet. Keep in
    /// sync with `managedGitignoreBlock` — a new entry belongs in BOTH: the block
    /// serves new projects, this list retro-fits existing ones.
    private static let managedGitignoreUpgrades: [(sentinel: String, lines: String)] = [
        (".agents/", """
        ...
        """),
        ("system/loop-runs/", """
        # Loop run history — regenerated by every run; the Loop's CONTRACT
        # (system/loop.json) is committed, its run log is not.
        system/loop-runs/
        """)
    ]
    ...
    *.partial.md
    ...
    /// The block a NEW project receives verbatim. Exposed read-only so a test can
    /// assert what is (and is not) ignored — `system/loop-runs/` must be, and
    /// `system/loop.json` must not, since the contract is committed on purpose.
    static var managedGitignoreBlockForTesting: String { managedGitignoreBlock }

    /// `existing` with every missing managed entry injected before the closing
    /// marker, or `nil` when it is already up to date.
    ///
    /// Internal + pure so the retro-fit path is testable without scaffolding a
    /// project on disk — it is the part that decides whether an existing checkout
    /// starts ignoring a newly-managed path, which is silent when wrong.
    ///
    /// Every missing entry is applied, not just the first: the original form was a
    /// single `if`, so a scaffold two versions behind could only ever catch up by
    /// one and stayed permanently behind on the rest.
    static func upgradedManagedGitignore(_ existing: String) -> String? {
        var injected = existing
        for upgrade in managedGitignoreUpgrades where !injected.contains(upgrade.sentinel) {
            ...
            for close in closeMarkers where injected.contains(close) {
                injected = injected.replacingOccurrences(
                    of: close, with: upgrade.lines + "\n" + close)
                break
            }
        }
        return injected == existing ? nil : injected
    }
    ...
    private static func ensureRootGitignore(at folderURL: URL) {
        ...
        if AppIdentity.isManagedGitignoreBlockPresent(existing) {
            if let injected = upgradedManagedGitignore(existing) {
