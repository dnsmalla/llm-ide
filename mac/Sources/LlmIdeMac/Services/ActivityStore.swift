...
final class ActivityStore {
    ...
    // On the first poll after launch we treat everything already on the
    ...
    func refresh() async {
        ...
        do {
            ...
            lastId = resp.lastId
            if !didInitialSeen {
                // First poll after launch: anchor the seen-cursor at the
                // server's current max id so pre-launch activity never lights
                // the badge. markSeen() POSTs uptoId = lastId; the badge stays
                // 0 until genuinely new activity arrives this session.
                didInitialSeen = true
                unreadCount = 0
                markSeen()
            } else {
