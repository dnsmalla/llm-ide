...
final class GitLabClient {
    ...
    enum GitLabError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .httpError(let code, let msg):
                // 403 on a write usually means the token lacks the `api` scope
                // (read-only `read_api` can browse but not create). 401 means
                // the token is invalid/expired. Make both actionable.
                if code == 403 {
                    return "GitLab denied this action (403): your token likely lacks the "
                        + "`api` scope (read-only tokens can browse but not create issues). "
                        + "Recreate the token with the `api` scope and update it in Settings → GitLab."
                }
                if code == 401 {
                    return "GitLab rejected the token (401): it's invalid or expired. "
                        + "Create a new token (with the `api` scope) in Settings → GitLab."
                }
    ...
    /// Bridge so the RepoBackend adapter (a separate-file extension that can't
    /// see `private let config`) can read the projects the user added in
    ...
    func fetchAllIssues(projectId: Int) async throws -> [GitLabIssue] {
        ...
        let reported = Int(http.value(forHTTPHeaderField: "X-Total-Pages") ?? "1") ?? 1
        // Cap total pages so a project reporting thousands of pages can't spawn
        // an unbounded fan-out (memory + connection blowup + rate-limit trip).
        // 50 pages × 100/page = 5,000 issues, plenty for the timeline/board.
        let maxPages = 50
        let totalPages = min(reported, maxPages)
        guard totalPages > 1 else { return firstBatch }

        // Pages 2…N, fetched in bounded chunks (not all at once) so we stay
        // under GitLab's rate limiter and the per-host connection cap.
        var all = firstBatch
        let chunkSize = 5
        var page = 2
        while page <= totalPages {
            let upper = min(page + chunkSize - 1, totalPages)
                ...
                for p in page...upper {
                    group.addTask { [baseQuery] in
                        try await self.get("/projects/\(projectId)/issues",
                                           query: baseQuery + [.init(name: "page", value: "\(p)")])
                    }
                }
    ...
    private func executeWithHeaders<T: Decodable>(_ req: URLRequest) async throws -> (T, HTTPURLResponse) {
        ...
        guard (200..<300).contains(http.statusCode) else {
            // Surface only the parsed `message` field (mirrors GitHubClient) —
            // never echo the raw body, which can carry unbounded content. Redact
            // as defense-in-depth before the error reaches UI/logs.
            let jsonMsg = (try? AppJSON.decoder.decode([String: String].self, from: data))?["message"]
            let msg = SecretRedactor.redact(jsonMsg.map { "HTTP \(http.statusCode): \($0)" }
