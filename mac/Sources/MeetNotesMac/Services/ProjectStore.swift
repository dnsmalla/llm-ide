...
final class ProjectStore: ObservableObject {
    ...
    private let log = Logger(subsystem: "com.meetnotes.macapp", category: "ProjectStore")

    /// Injected by `MeetNotesMacApp.init()` so the exporter can call
    ...
    var _apiClient: MeetNotesAPIClient?
    ...
    func openFolder(at url: URL) throws {
        ...
        // is non-empty and missing the required MeetNotes sub-folder tree.
...
enum ProjectStoreError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidFolderStructure(let name):
            return """
                "\(name)" is not a MeetNotes project.

                A valid project folder must contain meetings/, notes/, and plans/ sub-folders, \
                or have been created by MeetNotes. \
                Select an existing MeetNotes project folder, or choose an empty folder to start a new project.
                """
        }
    }
}
