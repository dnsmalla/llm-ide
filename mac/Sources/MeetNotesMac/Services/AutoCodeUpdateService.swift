...
final class AutoCodeUpdateService: ObservableObject {
    ...
    private let log = Logger(subsystem: "com.meetnotes.macapp", category: "AutoCodeUpdateService")
    ...
    init(config: AppConfig, backend: RepoBackend? = nil, registry: ProcessedActionsRegistry,
         projectStore: ProjectStore? = nil, api: MeetNotesAPIClient? = nil) {
        self.config = config
    ...
    func run() async {
        ...
        // The SQLite index lives in the project root's .meetnotes/ (not in
        // meetings/.meetnotes/) when a project is open — mirror AppEnvironment's
        ...
        let indexURL = indexRoot.appendingPathComponent(".meetnotes/index.sqlite")
