...
final class AppEnvironment {
    ...
    ///   the `.meetnotes/index.sqlite` file.  Pass the **project root** when
    ///   a project is open so the index lands in `project/.meetnotes/` (the
    ///   canonical, scaffolded location) instead of `meetings/.meetnotes/`.
    ...
    init(indexRootURL: URL? = nil) throws {
        ...
        // Place the SQLite index at <indexRoot>/.meetnotes/index.sqlite.
        // When a project is open indexRootURL is the project root, so the
        // index lives in the scaffolded .meetnotes/ dir alongside project.json
        // and sync.json — not buried inside meetings/.meetnotes/.
        let idxRoot = indexRootURL ?? folder
        try FileManager.default.createDirectory(
            at: idxRoot.appendingPathComponent(".meetnotes"),
            withIntermediateDirectories: true)
        let idx = try MeetingIndex(
            url: idxRoot.appendingPathComponent(".meetnotes/index.sqlite"))
