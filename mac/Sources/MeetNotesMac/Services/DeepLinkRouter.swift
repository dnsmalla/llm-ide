...
/// Routes incoming `meetnotes://` URLs to the right tab and brings the
...
final class DeepLinkRouter: ObservableObject {
    ...
    private let log = Logger(subsystem: "com.meetnotes.macapp", category: "DeepLink")

    func handle(_ url: URL) {
        guard url.scheme?.lowercased() == "meetnotes" else {
            log.warning("ignoring non-meetnotes URL: \(url, privacy: .public)")
            return
        }
        ...
        // Tab is the first segment.  meetnotes://transcript / -plan /
        ...
        // state.  An unvalidated value from a hostile meetnotes://
