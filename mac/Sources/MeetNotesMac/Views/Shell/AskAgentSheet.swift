...
struct AskAgentSheet: View {
    ...
    @State private var transcript: [MeetNotesAPIClient.AgentAskMessage] = []
    ...
    private let log = Logger(subsystem: "com.meetnotes.macapp", category: "AskAgentSheet")
    ...
    private func bubble(for msg: MeetNotesAPIClient.AgentAskMessage) -> some View {
    ...
    private func send() async {
        ...
        let userMsg = MeetNotesAPIClient.AgentAskMessage(role: .user, content: text)
    ...
    private func loadHistory() async {
        ...
        do {
            ...
            transcript = items.map { item in
                let role: MeetNotesAPIClient.AgentAskMessage.Role =
                    (item.role == "assistant") ? .assistant : .user
                return MeetNotesAPIClient.AgentAskMessage(role: role, content: item.content)
            }
        } catch {
