...
struct ChatSession: Identifiable, Codable, Equatable {
    ...
    let createdAt: Date
    var lastUsedAt: Date
    var history: [MeetNotesAPIClient.CodeAssistTurn]

    init(id: UUID = UUID(),
         title: String = "New chat",
         createdAt: Date = Date(),
         lastUsedAt: Date = Date(),
         history: [MeetNotesAPIClient.CodeAssistTurn] = []) {
        self.id = id
        self.title = title
    ...
    init(from decoder: Decoder) throws {
        ...
        self.history = try c.decode([MeetNotesAPIClient.CodeAssistTurn].self, forKey: .history)
    }
}
