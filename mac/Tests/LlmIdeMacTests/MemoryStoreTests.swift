...
struct MemoryStoreTests {
    ...
    @Test func seedIfMissingCreatesDirAndRepoTemplate() throws {
        ...
        let memoryDir = repo.appendingPathComponent("system")
    ...
    @Test func seedIsIdempotent() throws {
        ...
        try store.seedIfMissing(in: repo)
        let repoMd = repo.appendingPathComponent("system/repo.md")
    ...
    @Test func listFaultsAndQAReturnMarkdownFilesSorted() throws {
        let repo = try tmpRepoDir()
        defer { try? FileManager.default.removeItem(at: repo) }
        let faults = repo.appendingPathComponent("system/faults", isDirectory: true)
        let qa   = repo.appendingPathComponent("system/q&a", isDirectory: true)
    ...
    @Test func seedMigratesLegacyBugsDirToFaults() throws {
        ...
        let legacy = repo.appendingPathComponent("system/bugs", isDirectory: true)
        ...
        let faults = repo.appendingPathComponent("system/faults")
