...
final class ExplorerDragPayloadTests: XCTestCase {
    ...
    /// they must be rejected by `encode` for the same reason — otherwise
    /// `decode`'s `\.isNewline` split would tear the path in two.
    func testPathsContainingExoticNewlinesAreAlsoDropped() {
        let cr = URL(fileURLWithPath: "/tmp/we\rird.txt")
        let lineSep = URL(fileURLWithPath: "/tmp/we\u{2028}ird.txt")
        XCTAssertTrue(ExplorerDragPayload.encode([cr]).isEmpty)
        XCTAssertTrue(ExplorerDragPayload.encode([lineSep]).isEmpty)
    }
}
