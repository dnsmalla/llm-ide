...
struct CodeAssistantAgentContextTests {
    ...
    @Test func gitlabKindMapsToUrlAndBranchUnchanged() {
        ...
        #expect(p?.provider == "GitLab")
    }

    // MARK: - Config fallback (Settings → GitLab/GitHub without a workspace linkedRepo)

    private func freshConfig() -> AppConfig {
        AppConfig(userDefaults: UserDefaults(suiteName: "agentctx-\(UUID().uuidString)")!)
    }

    @Test func configFallbackNilWhenNothingConfigured() {
        #expect(CodeAssistantPanel.deriveActiveProject(fromConfig: freshConfig()) == nil)
    }

    @Test func configFallbackSurfacesActiveGitHubRepo() {
        let cfg = freshConfig()
        cfg.gitHubToken = "ghp_test_token"
        var r = SavedGitHubRepo(url: "acme/web", displayName: "Web", resolvedId: 7, isActive: true)
        r.defaultBranch = "main"
        cfg.gitHubSavedRepos = [r]
        let p = CodeAssistantPanel.deriveActiveProject(fromConfig: cfg)
        #expect(p?.name == "Web")
        #expect(p?.url == "acme/web")
        #expect(p?.defaultBranch == "main")
        #expect(p?.provider == "GitHub")
    }

    @Test func configFallbackPrefersGitLabWhenBothActive() {
        let cfg = freshConfig()
        cfg.gitLabToken = "glpat_test"
        cfg.gitHubToken = "ghp_test_token"
        cfg.gitLabSavedProjects = [SavedGitLabProject(
            url: "https://gitlab.com/acme/app", displayName: "App", resolvedId: 42, isActive: true)]
        cfg.gitHubSavedRepos = [SavedGitHubRepo(
            url: "acme/web", displayName: "Web", resolvedId: 7, isActive: true)]
        let p = CodeAssistantPanel.deriveActiveProject(fromConfig: cfg)
        #expect(p?.provider == "GitLab")
        #expect(p?.name == "App")
    }

    @Test func configFallbackIgnoresGitHubRepoWithoutToken() {
        let cfg = freshConfig()
        cfg.gitHubSavedRepos = [SavedGitHubRepo(
            url: "acme/web", displayName: "Web", resolvedId: 7, isActive: true)]
        // No gitHubToken set → not actionable, so not surfaced.
        #expect(CodeAssistantPanel.deriveActiveProject(fromConfig: cfg) == nil)
    }
}
