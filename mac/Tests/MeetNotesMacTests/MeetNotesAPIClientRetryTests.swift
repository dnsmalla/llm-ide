...
/// Exercises MeetNotesAPIClient.send()'s retry/error behavior through the
/// injectable fetch seam — no real network.
struct MeetNotesAPIClientRetryTests {
    ...
    @Test func getRetriesTransientStatusThenSucceeds() async throws {
        let counter = Counter()
        let client = MeetNotesAPIClient(baseURL: "https://example.test") { req in
